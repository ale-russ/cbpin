...
import { Users, UserPlus, Shield, Trash2, Mail, CheckCircle, Activity, Calendar, Lock } from 'lucide-react';
import { systemStateInstance, TeamMember } from '../../../data/sampleDataset';

export default function TeamManagement() {
  const [team, setTeam] = useState<TeamMember[]>(systemStateInstance.teamMembers);
  const [inviteName, setInviteName] = useState<string>("");
  const [inviteEmail, setInviteEmail] = useState<string>("");
  const [inviteRole, setInviteRole] = useState<'Guest' | 'SME User' | 'Professional User' | 'Enterprise User' | 'Administrator'>("Professional User");
  ...
  const [sessionLogs, setSessionLogs] = useState([
    { id: "SESS-1", device: "Security Shell Chrome, Mac OS X", location: "Kampala, Uganda", ip: "197.239.4.12", pingTime: "Connected 4m ago" },
    { id: "SESS-2", device: "Mobile App Mobilizer, Android", location: "Nairobi, Kenya", ip: "105.12.89.41", pingTime: "Connected 2h ago" },
    { id: "SESS-3", device: "Consortium Secure Terminal Linux", location: "Dar es Salaam, Tanzania", ip: "41.220.10.8", pingTime: "Connected 1d ago" }
  ]);
  ...
  const handleRevokeSession = (sessionId: string, deviceName: string) => {
    ...
    setNotification(`🚨 SECURITY KEY DISPATCH REFUSED: SESSION TERMINATED FOR ${deviceName}`);
  ...
  const permissionsMatrix = [
    { cap: "View Limited Tenders", guest: true, sme: true, pro: true, ent: true, admin: true },
    { cap: "Search full 500+ Indices", guest: false, sme: true, pro: true, ent: true, admin: true },
    { cap: "Receive SMS/Email Alerts", guest: false, sme: true, pro: true, ent: true, admin: true },
    { cap: "Inspect Opportunity Match Scores", guest: false, sme: false, pro: true, ent: true, admin: true },
    { cap: "Access Regional Predictive Forecasts", guest: false, sme: false, pro: true, ent: true, admin: true },
    { cap: "Geospatial Spatial Transit Maps", guest: false, sme: false, pro: false, ent: true, admin: true },
    { cap: "B2B Subcontractor Graph Relations", guest: false, sme: false, pro: false, ent: true, admin: true },
    { cap: "System Audit Logs Override", guest: false, sme: false, pro: false, guestAllowed: false, admin: true }
  ];
    ...
    <div className="space-y-6">
      ...
      {notification && (
        <div className="bg-emerald-950 border border-emerald-400 p-4 rounded-xl flex items-center justify-between text-xs font-mono text-emerald-400">
          ...
          <span className="text-slate-500 text-[10px]">VERIFIED GATEWAY APPROVED</span>
        </div>
      )}
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-slate-500">Corporate Seat Administrator</span>
            </div>
            <h2 className="text-lg font-bold text-slate-100 mt-1">Multi-Seat Team Management Matrix</h2>
            <p className="text-xs text-slate-400 mt-1 max-w-2xl">
              Invite teammates, assign granular role weights, check verification status, and audit active operations in real-time. Enterprise seats allow up to 5 concurrent operator logs.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-8 bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col justify-between min-h-[550px]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-left text-slate-300">
                ...
                <tbody className="divide-y divide-slate-850">
                  {team.map((mem) => (
                    <tr key={mem.id} className="hover:bg-slate-850/40 transition">
                      <td className="px-4 py-3 font-semibold text-slate-200">{mem.name}</td>
                      <td className="px-4 py-3 font-mono text-slate-400 text-[11px]">{mem.email}</td>
                      <td className="px-4 py-3">
                        <select
                          value={mem.role}
                          onChange={(e) => handleRoleToggle(mem.id, e.target.value as any)}
                      ...
                      <td className="px-4 py-3 text-slate-500 font-mono text-[10px]">{mem.addedAt}</td>
          ...
          <div className="mt-6 bg-slate-950/80 p-4 rounded-xl border border-slate-850 space-y-3 font-mono text-xs">
            <div className="flex justify-between items-center pb-2 border-b border-slate-900 select-none">
              <span className="text-[10px] text-slate-500 font-extrabold uppercase">● Concurrently Logged Devices Directory Audit</span>
              <span className="text-[9px] text-[#00FF00] font-bold animate-pulse">GATEWAY SHELL SHIELD SYNCED</span>
            </div>

            {sessionLogs.length === 0 ? (
              <p className="text-slate-500 text-center py-2 text-[11px]">No active concurrent keys mapped. Strict isolated state secure.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {sessionLogs.map((sess) => (
                  <div key={sess.id} className="p-2.5 bg-slate-900 rounded border border-slate-855 flex flex-col justify-between hover:border-amber-900/30 transition">
                    <div>
                      <div className="flex justify-between items-start text-[9.5px]">
                        <strong className="text-slate-200 truncate pr-2" title={sess.device}>{sess.device}</strong>
                        <span className="text-slate-500 shrink-0 text-[9px]">{sess.pingTime}</span>
                      </div>
                      <span className="text-[10.5px] text-slate-450 block mt-1">{sess.location}</span>
                      <code className="text-[9.5px] text-[#FF6321] block mt-1">{sess.ip}</code>
                    </div>
                    <button
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          {/* Invite form */}
          <form onSubmit={handleInvite} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-4">
            ...
            <div className="space-y-1.5 text-xs">
              <label className="text-slate-500 font-mono uppercase">Full Legal Name</label>
              <input
                type="text"
                required
                value={inviteName}
            ...
            <div className="space-y-1.5 text-xs">
              <label className="text-slate-500 font-mono uppercase">Master Business Email</label>
              <input
                type="email"
                required
                value={inviteEmail}
            ...
            <div className="space-y-2 text-xs">
              <label className="text-slate-500 font-mono uppercase block">Authority Level</label>
              <select
                value={inviteRole}
              ...
              >
                ...
                <option value="Professional User">Professional User (+Forecasts)</option>
                <option value="Enterprise User">Enterprise User (+Map & Graphs)</option>
                <option value="Administrator">Administrator (Terminal Admin)</option>
              </select>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 font-mono text-xs text-slate-300 space-y-3">
            ...
            <div className="space-y-1">
              <div className="flex justify-between font-bold text-slate-150">
                ...
                <span className="text-emerald-400">{allocatedSeatsCount} Seats</span>
              </div>
              <input
                type="range"
                min="5"
                ...
                onChange={(e) => setAllocatedSeatsCount(parseInt(e.target.value))}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <div className="space-y-2.5">
              {permissionsMatrix.slice(0, 5).map((pm, idx) => (
                <div key={idx} className="flex justify-between items-center text-[11px] border-b border-slate-850/60 pb-1.5">
                  <span className="text-slate-400 font-mono leading-tight">{pm.cap}</span>
                  <div className="flex gap-1">
                    <span className={`px-1 py-0.5 rounded text-[8px] font-mono ${pm.sme ? 'bg-emerald-950/30 text-emerald-400 border border-emerald-900/40' : 'bg-slate-950 text-slate-600'}`}>SME</span>
                    <span className={`px-1 py-0.5 rounded text-[8px] font-mono ${pm.pro ? 'bg-amber-950/30 text-amber-500 border border-amber-900/40' : 'bg-slate-950 text-slate-600'}`}>PRO</span>
                    <span className={`px-1 py-0.5 rounded text-[8px] font-mono ${pm.ent ? 'bg-violet-950/30 text-violet-400 border border-violet-900/40' : 'bg-slate-950 text-slate-600'}`}>ENT</span>
                  </div>
                </div>
              ))}
            </div>
