...
import { User, Building, ShieldCheck, Mail, MapPin, Award, Calendar, BarChart2, CheckSquare, Layers, Clock, Activity, Edit2 } from 'lucide-react';
...
export default function EnterpriseProfile({ currentRole, savedOpportunities, onUnsaveOpportunity, onNavigateToView }: EnterpriseProfileProps) {
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [fullName, setFullName] = useState<string>("Sarah Alubas");
  const [companyName, setCompanyName] = useState<string>("Mugerwa Engineering & Logistics Ltd");
  ...
  const [interests, setInterests] = useState<string[]>(["Cross-Border Heavy Trucking", "Oil & Gas Roadbeds", "Sovereign Logistics Corridor Grants"]);
  ...
  const savedOppsDetails = systemStateInstance.opportunities.filter(o => savedOpportunities.includes(o.id));
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              <span className="absolute bottom-0 right-0 w-4 h-4 bg-emerald-400 border-2 border-slate-900 rounded-full" title="SaaS Sync Online"></span>
            </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-slate-500">MUGERWA-LOGISTICS TENANT</span>
              </div>
              <h2 className="text-xl font-bold text-slate-100 mt-1">{fullName}</h2>
              <p className="text-xs text-slate-400 mt-1">{companyName} // Lead Procurement Officer</p>
            </div>
          </div>

          <div className="flex gap-4">
            <div className="bg-slate-950 px-5 py-2 border border-slate-850 rounded-xl text-center">
              <span className="text-[9px] text-slate-500 block uppercase font-mono font-bold">Profile completion</span>
              <strong className="text-lg font-bold text-[#00FF00] font-mono">{profileCompletionPercent}%</strong>
              <div className="w-24 bg-slate-850 h-1 rounded-full overflow-hidden mt-1 mx-auto">
                <div className="bg-[#00FF00] h-full" style={{ width: `${profileCompletionPercent}%` }}></div>
              </div>
            </div>
            ...
            >
              ...
              {isEditing ? "Close Credentials Drawer" : "Edit Operational Profile"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5 text-xs">
                  <label className="text-slate-500 font-mono">FULL OPERATOR NAME</label>
                  <input
                    type="text"
                    required
                    value={fullName}
                ...
                <div className="space-y-1.5 text-xs">
                  <label className="text-slate-500 font-mono">COMPANY INCORPORATION NAME</label>
                  <input
                    type="text"
                    required
                    value={companyName}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1.5 text-xs">
                  <label className="text-slate-500 font-mono">PRIMARY SECTOR</label>
                  <select
                    value={industry}
                ...
                <div className="space-y-1.5 text-xs">
                  <label className="text-slate-500 font-mono">REGISTRY COUNTRY</label>
                  <select
                    value={country}
                ...
                <div className="space-y-1.5 text-xs">
                  <label className="text-slate-500 font-mono">YEARS IN SECTOR (MINIMUMS)</label>
                  <input
                    type="number"
                    value={experience}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5 text-xs">
                  <label className="text-slate-500 font-mono">ANNUAL REVENUE RANGE</label>
                  <input
                    type="text"
                    value={revenueRange}
                ...
                <div className="space-y-1.5 text-xs">
                  <label className="text-slate-500 font-mono">ACTIVE EMPLOYEE headcount</label>
                  <input
                    type="number"
                    value={employeeCount}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-850">
                    <span className="text-[9px] text-slate-500 font-mono uppercase block">Sovereign Origin</span>
                    <strong className="text-xs text-slate-200 mt-1 block flex items-center gap-1">
                      <MapPin className="h-3 w-3 text-red-400" /> {country}
                    </strong>
                  </div>
                  <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-850">
                    <span className="text-[9px] text-slate-500 font-mono uppercase block">Sector Expertise</span>
                    <strong className="text-xs text-slate-200 mt-1 block truncate">{industry}</strong>
                  </div>
                  <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-850">
                    <span className="text-[9px] text-slate-500 font-mono uppercase block">Industry Standing</span>
                    <strong className="text-xs text-slate-200 mt-1 block font-mono">{experience} Years Track</strong>
                  </div>
                  <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-850">
                    <span className="text-[9px] text-slate-500 font-mono uppercase block">Revenue Sizing</span>
                    <strong className="text-xs text-[#00FF00] mt-1 block font-mono">{revenueRange}</strong>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                {savedOppsDetails.length === 0 ? (
                  <div className="p-8 bg-slate-950/20 border-2 border-dashed border-slate-850 rounded-xl text-center space-y-2">
                    ...
                    <p className="text-xs text-slate-400">No opportunities bookmarked yet.</p>
                    <button
                      onClick={() => onNavigateToView("Opportunities")}
                    ...
                    <table className="w-full text-xs text-left text-slate-300">
                      ...
                      <tbody className="divide-y divide-slate-850">
                        {savedOppsDetails.map((opp) => (
                          <tr key={opp.id} className="hover:bg-slate-900/40 transition">
                            <td className="px-4 py-3 font-mono text-slate-100 font-bold">{opp.id}</td>
                            <td className="px-4 py-3 max-w-xs truncate text-slate-300 font-medium">{opp.title}</td>
                            <td className="px-4 py-3 text-slate-450 font-mono text-[11px]">{opp.country}</td>
                            <td className="px-4 py-3 text-[#00FF00] font-mono font-bold">${(opp.budgetUSD / 1000000).toFixed(2)}M</td>
                            <td className="px-4 py-3 text-right space-x-1.5">
                              <button
                                onClick={() => onNavigateToView("Opportunities", opp)}
        ...
        <div className="xl:col-span-4 bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-6 flex flex-col justify-between h-full min-h-[500px]">
          ...
          <div className="space-y-6">
            ...
            <div>
                ...
                {certifications.map((cert) => (
                  ...
                  >
                    <div className="flex gap-2 items-start">
                      ...
                      <span className="text-slate-300 leading-tight">{cert}</span>
                    </div>
                    <button 
              ...
              <div className="flex gap-2 text-xs mt-3">
                ...
                <button
                  ...
                  className="px-3 bg-slate-950 text-slate-300 border border-slate-800 hover:text-[#FF6321] hover:border-[#FF6321]/50 rounded-lg font-mono text-xs"
                >
                  + Add
                </button>
              </div>
            ...
            <div>
              ...
              <div className="bg-slate-950/40 p-3.5 rounded-xl border border-slate-850 space-y-3 font-mono text-[11px] text-slate-400">
                <div className="flex justify-between"><span className="text-slate-500">Security Ring:</span> <span className="text-[#00FF00]">ONLINE // EAC PORTAL</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Node Location:</span> <span className="text-slate-200">Kampala Central</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Verification Hash:</span> <span className="text-sky-400">CPIN-240-EAFRICA</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Licensing Plan:</span> <span className="text-amber-500">{currentRole}</span></div>
                <div className="flex justify-between"><span className="text-slate-500">Registered seats:</span> <span className="text-slate-200">3 of 5 seats used</span></div>
              </div>
            </div>
          ...
          <div className="bg-slate-950 p-3 rounded-lg border border-slate-850 text-[10px] text-slate-500 font-mono leading-relaxed mt-auto">
            ⚡ CPIN verified contractors are pre-screened under standard OECD Anti-bribery guidelines and legal registration directives.
          </div>
