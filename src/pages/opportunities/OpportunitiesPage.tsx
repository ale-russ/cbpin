...
import { Search, SlidersHorizontal, MapPin, DollarSign, Calendar, Sliders, Eye } from 'lucide-react';
...
export default function OpportunitiesPage() {
  ...
  const uniqueCountries = ["All", "Uganda", "Kenya", "Tanzania", "Rwanda", "South Sudan", "DRC"];
  const uniqueSectors = [
    "All",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900 border border-slate-800 p-5 rounded-xl">
        <div>
          <span className="text-[10px] font-mono text-[#FF6321] font-bold block uppercase tracking-wider">ELASTIC DECI-INDEX ENGINE</span>
          <h1 className="text-xl font-bold text-slate-100">Procurement Opportunities</h1>
          <p className="text-xs text-slate-400 mt-1">
            Real-time cross-border index querying across East Africa. Select a tender to open intelligence dossiers.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 bg-slate-950 border border-slate-850 p-4 rounded-xl space-y-5 h-fit">
          ...
          <div className="space-y-1 text-xs">
            <label className="text-slate-500 font-mono font-bold block uppercase scale-90 origin-left">HEADLESS QUERIES</label>
          ...
          {/* Country list */}
          <div className="space-y-1 text-xs">
            <label className="text-slate-500 font-mono font-bold block uppercase scale-90 origin-left">Sovereign Domain</label>
            <select
            ...
            >
              {uniqueCountries.map((c) => (
                <option key={c} value={c}>{c === 'All' ? 'All Sovereign (EAC)' : c}</option>
              ))}
            </select>
          </div>
          ...
          {/* Sector selection */}
          <div className="space-y-1 text-xs">
            <label className="text-slate-500 font-mono font-bold block uppercase scale-90 origin-left">Indicate Industry Sector</label>
            <select
            ...
            >
              {uniqueSectors.map((s) => (
                <option key={s} value={s}>{s === 'All' ? 'All Industrial Sectors' : s.replace(' & ', ' and ')}</option>
              ))}
            </select>
          </div>
          ...
          <div className="space-y-1 text-xs">
            <label className="text-slate-500 font-mono font-bold block uppercase scale-90 origin-left">Sovereign Val USD</label>
            <select
              value={valueBracket}
            ...
            >
              {valueBrackets.map((vb) => (
                <option key={vb} value={vb}>{vb === 'All' ? 'All Valuations' : vb}</option>
              ))}
            </select>
          </div>

          <div className="text-[10px] bg-slate-900 border border-slate-850 p-3 rounded-lg font-mono text-slate-400 space-y-1">
            <span className="font-bold text-[#FF6321] uppercase">Elastic Search Context</span>
            <p className="leading-snug scale-95 origin-left">
              Engine matched <strong>{searchResults.length}</strong> active pipelines under seat authority profile.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  {isGuestAndLocked && (
                    <div className="absolute inset-0 bg-slate-950/70 flex flex-col justify-center items-center font-mono z-10 text-center p-4">
                      <span className="font-bold text-[#FF6321]">🔐 SEAT INDEX LOCKED</span>
                      <p className="text-[10px] text-slate-500 scale-90 mt-1">Upgrade account plan to inspect full index</p>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-[10px] font-bold text-slate-500 uppercase">{opt.id}</span>
                      <span className="w-1.5 h-1.5 rounded-full bg-[#00ff00]"></span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className={`px-2 py-0.5 rounded font-mono text-[9px] font-bold ${
                        opt.recommendedAction === 'Pursue Immediately' ? 'text-emerald-400 bg-emerald-950/25 border border-emerald-900/30' :
                        opt.recommendedAction === 'Form Consortium' ? 'text-amber-400 bg-amber-950/25 border border-amber-900/30' :
                        'text-slate-400 bg-slate-900 border border-slate-850'
                      }`}>
                        {opt.recommendedAction === 'Pursue Immediately' ? '🎯 PURSUE' :
                         opt.recommendedAction === 'Form Consortium' ? '🤝 CONSORT' :
                         opt.recommendedAction === 'Review Further' ? '🔍 REVIEW' : '❌ AVOID'}
                      </span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className={`text-sm cursor-pointer p-1 transition ${
                          isSaved ? 'text-amber-500 hover:text-slate-400' : 'text-slate-600 hover:text-[#FF6321]'
                        }`}
                        title={isSaved ? "Saved to Bookmark Ledger" : "Add to Bookmarks"}
                      >
                        ★
                      </button>
                    </div>
                  ...
                  <div className="flex justify-between items-center pt-2.5 border-t border-slate-850 text-[10px] font-mono text-slate-400 mt-2">
                    ...
                    <div className="flex items-center gap-1.5 text-slate-100 font-black">
                      <span className="text-emerald-400">${(opt.budgetUSD / 1000000).toFixed(2)}M</span>
            ...
            {searchResults.length === 0 && (
              <div className="col-span-2 text-center py-16 bg-slate-950/50 rounded-2xl border border-slate-850 border-dashed text-slate-500 font-mono text-xs">
                No matching tenders found under these parameters. Try removing search limits.
              </div>
            )}
          </div>
